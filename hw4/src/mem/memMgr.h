...
#define toSizeT(t)      ( (t % SIZE_T) ? ((t / SIZE_T + 1) * SIZE_T) : t ) // TODO
//
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) { //reference to pointer ret of type T
      // TODO
      size_t n = toSizeT(t);
      if (n > getRemainSize()) {
         return false;  
      } else {
         ret = (T*)_ptr; //char* to T*
         _ptr += n;
         return true;
      }
   }

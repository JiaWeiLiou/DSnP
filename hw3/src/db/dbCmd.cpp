...
{  
   ...
   if (myStrNCmp("-Row", tokens[0], 1) == 0) {
      int rowNum = 0;
      if (tokens.size() == 1) {
         return CmdExec::errorOption(CMD_OPT_MISSING, tokens[0]);
      } else if (tokens.size() > 2) {
         return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[2]);
      } else if (!(checkRowIdx(tokens[1], rowNum))) {
         return CMD_EXEC_ERROR;        
      } else {
         cout << dbtbl[rowNum] << endl;
      }
   } else if (myStrNCmp("-Column", tokens[0], 1) == 0) {
      int colNum = 0;
      if (tokens.size() == 1) {
         return CmdExec::errorOption(CMD_OPT_MISSING, tokens[0]);
      } else if (tokens.size() > 2) {
         return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[2]);
      } else if (!(checkColIdx(tokens[1], colNum))) {
         return CMD_EXEC_ERROR;        
      } else {
         dbtbl.printCol(colNum);;
      }           
   } else if (myStrNCmp("-Table", tokens[0], 1) == 0) {
      if (tokens.size() == 1) {
         cout << dbtbl << endl;
      } else if (tokens.size() > 1) {
         return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[1]);
      }
   } else if (myStrNCmp("-Summary", tokens[0], 1) == 0) {
      if (tokens.size() == 1) {
         dbtbl.printSummary();
      } else if (tokens.size() > 1) {
         return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[1]);
      ...
      if (!(checkRowIdx(tokens[0], rowNum))) {
         return CMD_EXEC_ERROR; 
      } else if (!(checkColIdx(tokens[1], colNum))) {
         return CMD_EXEC_ERROR;        
      } else {
         if (dbtbl[rowNum][colNum] == INT_MAX) {
            cout << "null" << endl;        
         } else {
            cout << dbtbl[rowNum][colNum] << endl;
         }
      }
   } else {
      return CmdExec::errorOption(CMD_OPT_ILLEGAL, tokens[0]);

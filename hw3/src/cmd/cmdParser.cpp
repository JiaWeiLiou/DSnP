...
void CmdParser::listCmd(const string& str)
{
   ...
   if(str.empty() || str == " ") {
      ...
      for (size_t i = 0, cnt = 0; i < cmdNames.size(); ++i, ++cnt) {
         if (cnt % 5 == 0) {
            cout << endl;
         }
         cout << setw(12) << left << cmdNames[i];
      }
      reprintCmd();
      ...
      string token;
      if (myStrGetTok(str, token) == std::string::npos) {

         vector<string> matchStr;
         for (size_t i = 0; i < cmdNames.size(); ++i) {
            if (str.size() <= cmdNames[i].size()) {
               if (myStrNCmp(cmdNames[i], str, str.size()) == 0) {
                  matchStr.push_back(cmdNames[i]);
               }
            } else {
               mybeep();
            }
         ...
         // Condition 2
         if (matchStr.size() > 1) {
            for (size_t i = 0, cnt = 0; i < matchStr.size(); ++i, ++cnt) {
               if (cnt % 5 == 0) {
                  cout << endl;
               }
               cout << setw(12) << left << matchStr[i];
            }
            reprintCmd();
         // Condition 3
         } else if (matchStr.size() == 1) {
            string strOut = matchStr[0].substr(str.size());
            for (size_t i = 0; i < strOut.size(); ++i) {
               insertChar((char)strOut[i]);
            }
            insertChar(' ');
            _tabPressCount = 0;
         // Condition 4
         ...
         CmdExec* helpPrint = getCmd(token);
         // Condition 5
         if (_tabPressCount == 1 && helpPrint != 0) {
            cout << endl;
            helpPrint->usage(cout);
            reprintCmd();
         // Condition 6
         } else if (_tabPressCount > 1 && helpPrint != 0) {
            string filedir = ".";
            string prefix = str.substr(str.find_last_of(' ') + 1);
            size_t dirpos = prefix.find_last_of('/');
            if (dirpos != string::npos) {
               filedir = prefix.substr(0, dirpos);
               prefix = prefix.substr(dirpos + 1);
            }
            vector<string> matchFile;
            listDir(matchFile, prefix, filedir); 
            // Condition 6.1
            if (prefix.size() == 0) {
               for (size_t i = 0, cnt = 0; i < matchFile.size(); ++i, ++cnt) {
                  if (cnt % 5 == 0) {
                     cout << endl;
                  }
                  cout << setw(16) << left << matchFile[i];
               }
               reprintCmd();
            } else if (matchFile.size() > 1) {
               string comPrefix = prefix;
               string comFile = matchFile[0];
               bool comflag = 1;
               for (size_t i = prefix.size(); i < comFile.size(); ++i) {
                  for (size_t j = 1; j < matchFile.size(); ++j) {
                     if (comFile[i] != matchFile[j][i]) {
                        comflag = 0;
                        break;
                     } 
                  }
               ...
               // Condition 6.2
               if (comPrefix.size() == prefix.size()) {
                  for (size_t i = 0, cnt = 2; i < matchFile.size(); ++i, ++cnt) {
                     if (cnt % 5 == 0) {
                        cout << endl;
                     }
                     cout << setw(16) << left << matchFile[i];
                  ...
                  reprintCmd();
               // Condition 6.3
               } else if (comPrefix.size() > prefix.size()) {
                  for (size_t i = prefix.size(); i < comPrefix.size(); ++i) {
                     insertChar(comPrefix[i]);
                  }
               }
            // Condition 6.4
            } else if (matchFile.size() == 1) {
               string strOut = matchFile[0].substr(prefix.size());
               for (size_t i = 0; i < strOut.size(); ++i) {
                  insertChar((char)strOut[i]);
               }
               insertChar(' ');
...
CmdExec* CmdParser::getCmd(string cmd)
{
   ...
   // TODO...
   for(CmdMap::const_iterator i = _cmdMap.begin(); i != _cmdMap.end(); ++i) {
      string mandOpt = i->first + ((i->second)->getOptCmd());
      int num_compare = i->first.size();
      if(myStrNCmp(mandOpt, cmd, num_compare) == 0) {
         e = i->second;
      }
   }
   return e;
